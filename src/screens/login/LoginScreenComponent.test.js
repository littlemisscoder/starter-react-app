...
describe('LoginScreenComponent is authenticated ', () => {
  let wrapper;
  let props;
  ...
  it('renders with one Redirect', () => {
    //When
    wrapper = shallow(<LoginScreenComponent {...props} />);

    //Then
    expect(wrapper.find('Redirect')).toHaveLength(1);
  });

  it('renders with one Redirect last known location.state', () => {
    //When
    wrapper = shallow(<LoginScreenComponent {...props} />);
  ...
  it('renders with one Redirect to default path and last known state is undefined', () => {
    //Given
...
describe('LoginScreenComponent is not authenticated ', () => {
  let sandbox;
  let wrapper;
  let props;
  ...
  it('renders with one PageHeader having content "Log In"', () => {
    //When
    wrapper = shallow(<LoginScreenComponent {...props} />);

    //Then
    expect(wrapper.find('PageHeader').props().children).toBe('Log In');
  });

  it('renders without Alert when loginError is null', () => {
    //When
    wrapper = shallow(<LoginScreenComponent {...props} />);
  ...
  it('renders without Alert when loginError is empty string', () => {
    //Given
    props.loginError = '';
  ...
  it('renders one Alert with error message when loginError has length > 0', () => {
    //Given
    ...
    expect(wrapper.find('Alert').props().children).toEqual(props.loginError);
  });
  ...
  it('should update state.isLoading=true and call loginUser on submit', () => {
    //Given
    const state = {
      username: 'username',
      password: 'password',
    ...
    expect(loginUser.calledOnce).toBeTruthy();
  });

  it('renders Link with to=/register', () => {
    //When
    wrapper = shallow(<LoginScreenComponent {...props} />);
  ...
  it('componentWillReceiveProps should set isLoading=false when loginError changes', () => {
    //Given
    const updateProps = {
  ...
  it('componentWillReceiveProps should make no changes when loginError is the same', () => {
    //Given
    const updateProps = {
      loginError: null
    };
